...
OP_RETURN_BCH_FEE=0.000003 # BCH fee to pay per transaction
OP_RETURN_BCH_DUST=0.0000001 # omit BCH outputs smaller than this

OP_RETURN_MAX_BYTES=220 # maximum bytes in an OP_RETURN (80 as of Bitcoin 0.11)
...
def OP_RETURN_send(send_address, send_amount, metadata, testnet=False):
	...
	output_amount=send_amount+OP_RETURN_BCH_FEE
	...
	if change_amount>=OP_RETURN_BCH_DUST:
...
def OP_RETURN_store(data, testnet=False):
	...
	output_amount=OP_RETURN_BCH_FEE*int((data_len+OP_RETURN_MAX_BYTES-1)/OP_RETURN_MAX_BYTES) # number of transactions required
	...
	for data_ptr in range(0, data_len, OP_RETURN_MAX_BYTES):
		...
		change_amount=input_amount-OP_RETURN_BCH_FEE
		change_amount=round(change_amount,8)
		metadata=data[data_ptr:data_ptr+OP_RETURN_MAX_BYTES]
			
		# Build and send this transaction
	
		outputs={}
		if change_amount>=OP_RETURN_BCH_DUST: # might be skipped for last transaction
			outputs[change_address]=change_amount
